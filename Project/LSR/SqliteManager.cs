...
namespace LSR
{
    public class SqliteManager
    {
        ...
        {
            // 获取day天前的日期
            string date_before = DateTime.Now.AddDays(-1*day).ToString("yyyy-MM-dd");
        ...
        public int unForgottenRate(int day)
        {
            ...
            string date_till_1stday_str = date_till_1stday.ToString("yyyy-MM-dd");
            string date_1stday_str = date_1stday.ToString("yyyy-MM-dd");
            string sql = $"SELECT COUNT(*) FROM RecitedWord WHERE Value<60 AND Date BETWEEN '{date_1stday_str}' AND '{date_till_1stday_str}'";
        ...
        //记忆持久度
        // thisday+day 天前背过的总量 - thisday天前， day天以内Value<60的单词数量
        public int lastingTime(int thisday, int day)
        {
            ...
            string sql = $"SELECT COUNT(DISTINCT Word) FROM RecitedWord WHERE Value<60 " +
        ...
        public void updateRecitedWords(string word, int identify)
        {
            ...
            string sql;
            if (identify == 0)
                sql = "UPDATE RecitedWord " +
                      "SET Identify = 0, value = 150";
            else
                sql = string.Format(@"UPDATE RecitedWord " +
                                       @"SET {0} = {1} + 1, value = value + {2}, IsReviewedToday = 1 " +
                                       @"WHERE word = '{3}'", coloum, coloum, Cmp, word
                                       );
            // 执行sql语句
            SQLiteCommand command = new SQLiteCommand(sql, db_connection);
            command.ExecuteNonQuery();
